...
namespace bot
{
    ...
    std::vector<MoveRequest> BotPlayer::collect_move_requests()
    {
        ...
        for (const auto &ship_pair : me->ships)
        {
            ...
            // Rechercher ou creer le ShipFSM pour ce ship
            ...
            // Drop le plus proche de ce ship
    ...
    bool BotPlayer::try_build_dropoff(std::vector<hlt::Command> &commands)
    {
        ...
        {
            // Annuler tout les plans de dropoff en cours en ENDGAME
        ...
        // Distance minimale entre le nouveau dropoff et les depots existants
        int min_depot_dist = std::max(8, game_map->width / constants::MIN_DROPOFF_DEPOT_DISTANCE_RATIO);

        // Reset du plan de dropoff si le ship assigné est mort
        ...
        // Si un plan de dropoff est en cours, tenter de le construire
        if (bb.planned_dropoff_pos.x >= 0 && bb.dropoff_ship_id >= 0)
        {
            auto ship_it = me->ships.find(bb.dropoff_ship_id);
            if (ship_it == me->ships.end())
                return false; // Le ship assigné est mort, on attend le prochain tour pour reset le plan

            auto ship = ship_it->second;
            hlt::Position target = bb.planned_dropoff_pos;

            // Si le ship est arrive a destination, tenter la conversion
            if (ship->position == target)
            {
                // Calcul du cout reel de conversion en prenant en compte le halite sur le ship et la cell
            ...
            // Pas encore arrive a destination
            bb.persistent_targets[ship->id] = target;
            return false;
        }

        // Pas de plan de dropoff en cours, en creer un si les conditions sont reunies

        // Pas assez de halite pour construire un dropoff
        if (me->halite < hlt::constants::DROPOFF_COST / 2)
            return false;

        // Trouver la meilleure pos
        std::vector<hlt::Position> depots = get_drops_positions();
        hlt::Position best_pos = bb.find_best_dropoff_position(*game_map, depots, min_depot_dist);
        if (best_pos.x < 0)
            return false;

        // Trouver le ship le plus proche de cette pos pour lui assigner la mission de dropoff
        std::shared_ptr<hlt::Ship> best_ship = nullptr;
        int best_dist = 9999;
        int max_assign_dist = game_map->width / 3;
                                  ...
                                  const std::vector<MoveResult> &results) const
    {
        ...
        // Pas assez de tours restants pour rentabiliser un nouveau ship
        if (turns_remaining < constants::SPAWN_MIN_TURNS_LEFT)
            return false;

        // Pas assez de halite pour spawn un nouveau ship
        if (me->halite < hlt::constants::SHIP_COST)
            return false;

        // Reserver de la place pour le dropoff en construction
        ...
        // Verif qu'on ne va pas spawn sur un ship qui bouge ou qui va rester sur place
